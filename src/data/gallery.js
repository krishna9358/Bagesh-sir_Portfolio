/* eslint-disable max-len */
const data = [

  {
    link:'/images/gallery/52.jpeg',
    caption:'Meeting of <strong>committee members </strong> and <strong>professor O.P vyas </strong> with <strong>renowned professors </strong> across the globe at <strong>IIIT Allahabad </strong',
  },
  {
    link: '/images/gallery/1.jpeg',
    caption: 'Research paper presentation at <strong>IIT BHU Varanasi India</strong> with <strong>Manipal University</strong> and <strong>IIIT students Allahabad</strong>',
  },
 
  {
    link: '/images/gallery/6.jpeg',
    caption: 'Discussion with Professor <strong>OP Vyas</strong> and <strong>PhD scholars</strong> at <strong>IIIT Allahabad</strong> on trending topics in <strong>process mining</strong>',
  },
 
  {
    link: '/images/gallery/8.jpeg',
    caption: 'Visit to <strong>IIT kanpur</strong> on discussion about <strong>future collaborations and research projects.</strong>',
  },
  {
    link:'/images/gallery/51.jpeg',
    caption:'Meeting of <strong>committee members </strong> and <strong>professor O.P vyas </strong> with <strong>renowned professors </strong> across the globe at <strong>IIIT Allahabad </strong',
  },
  {
    link: '/images/gallery/11.jpeg',
    caption: 'Research paper presentation at <strong>IIT BHU Varanasi India</strong> with <strong>Manipal University</strong> and <strong>IIIT students Allahabad</strong>',
  },
  {
    link: '/images/gallery/7.jpeg',
    caption: 'Visit to <strong>IIT kanpur</strong> on discussion about <strong>future collaborations and research projects.</strong>',
  },
  
  {
    link: '/images/gallery/29.jpg',
    caption: 'Industrial Visit at <strong>Google Bangalore</strong with <strong>IIIT Allahabad </strong Juniors Kalyan Kumar Dubey and Aayush Chaudhry about <strong>academic collaboration, placement opportunities and research projects.</strong',
  },
  {
    link: '/images/gallery/30.jpg',
    caption: 'Industrial Visit at <strong>Google Bangalore</strong with <strong>IIIT Allahabad </strong Juniors Kalyan Kumar Dubey and Aayush Chaudhry about <strong>academic collaboration, placement opportunities and research projects.</strong',
  },
  // Adding new images
  {
    link: '/images/gallery/31.jpeg',
    caption: 'Presenting <strong> Deep learning</strong>research paper at <strong> NIT Patna </strong>with <strong>IIIT Allahabad </strong> and <strong>Manipal University Jaipur </strong>students team',
  },
  {
    link: '/images/gallery/32.jpeg',
    caption: 'Presenting <strong> Deep learning </strong> research paper at <strong> NIT Patna </strong>with <strong>IIIT Allahabad </strong> and <strong>Manipal University Jaipur </strong>students team',
  },
  {
    link: '/images/gallery/33.jpeg',
    caption: 'Meeting with professor <strong> O.P Vyas  </strong>along with <strong>Manipal University jaipur  </strong>students at <strong>IIIT Allahabad </strong>',
  },
  {
    link: '/images/gallery/abhinav.png',
    caption:"Empowering Minds:<strong> Industry Leaders </strong>Inspire at <strong>Manipal University Jaipur's Expert Talk Series</strong> in Computer Science and Technology",
  },
  {
    link: '/images/gallery/aditya.png',
    caption:"Empowering Minds:<strong> Industry Leaders </strong>Inspire at <strong>Manipal University Jaipur's Expert Talk Series</strong> in Computer Science and Technology",
  },
  {
    link: '/images/gallery/aseem.png',
    caption:"Empowering Minds:<strong> Industry Leaders </strong>Inspire at <strong>Manipal University Jaipur's Expert Talk Series</strong> in Computer Science and Technology",
  },  {
    link: '/images/gallery/kalyan.png',
    caption:"Empowering Minds:<strong> Industry Leaders </strong>Inspire at <strong>Manipal University Jaipur's Expert Talk Series</strong> in Computer Science and Technology",
  },  {
    link: '/images/gallery/Luv.png',
    caption:"Empowering Minds:<strong> Industry Leaders </strong>Inspire at <strong>Manipal University Jaipur's Expert Talk Series</strong> in Computer Science and Technology",
  },
  {
    link: '/images/gallery/vaibhav.png',
    caption:"Empowering Minds:<strong> Industry Leaders </strong>Inspire at <strong>Manipal University Jaipur's Expert Talk Series</strong> in Computer Science and Technology",
  },
  {
    link: '/images/gallery/34.jpeg',
    caption: 'Industry Visit <strong>sprinkler</strong> team at <strong>gurgaon India </strong>with <strong>Manipal University students</strong>',
  },
  {
    link: '/images/gallery/35.jpeg',
    caption: '<strong>Research internship </strong>discussion on <strong>process mining </strong>with professor <strong>op vyas </strong>at <strong>Manipal University</strong>',
  },
  {
    link: '/images/gallery/36.jpeg',
    caption: "Encompassing ancient wisdom and modern learning, this is <strong>Nalanda University </strong>in <strong>Bihar, India.</strong> Considered one of the world's oldest universities, it flourished as a center of Buddhist studies for centuries.",
  },
  {
    link: '/images/gallery/37.jpeg',
    caption: 'Industrial visit at <strong>MNIT Jaipur </strong>to discuss robotics startups and Innovation and incubation centre with <strong>Manipal University </strong>students.  People brainstorm around a table, prototypes visible in background.',
  },
  {
    link: '/images/gallery/38.jpeg',
    caption: 'Winners at <strong>DEVFEST hackathon</strong> oraganised by<strong> Manipal University jaipur</strong>',
  },
  {
    link: '/images/gallery/39.jpeg',
    caption: 'Our team had the privilege of being guided by the insightful leadership of <strong>Subhajit Bhattacharya </strong>, Associate Vice President at Accenture.',
  },
  {
    link: '/images/gallery/40.jpeg',
    caption: 'Industrial visit  with <strong> Manipal University students</strong> and discussion with <strong>Shamse Alam( Senior Research staff member at Bharat research limited , Central research laboratory) Ex- Alumin IIIT Allahabad</strong>',
  },
  {
    link: '/images/gallery/41.jpeg',
    caption: 'Casual Interaction with <strong>Dr. CP Thakur </strong>sir( chancellor of <strong>Central University gaya</strong>',
  },
  {
    link: '/images/gallery/42.jpeg',
    caption: 'Casual Interaction with <strong>Dr. CP Thakur </strong>sir( chancellor of <strong>Central University gaya</strong>',
  },
  {
    link: '/images/gallery/43.jpg',
    caption: "Discussing upcoming research project with <strong>Bihar's IT Minister(Santosh Kumar Suman)</strong>, focusing on IT infrastructure development and advancement in the state.",
  },
  {
    link: '/images/gallery/44.jpg',
    caption: "Discussing upcoming research project with <strong>Bihar's IT Minister(Santosh Kumar Suman)</strong>, focusing on IT infrastructure development and advancement in the state.",
  },
  {
    link: '/images/gallery/45.jpg',
    caption: "Discussing upcoming research project with <strong>Bihar's IT Minister(Santosh Kumar Suman)</strong>, focusing on IT infrastructure development and advancement in the state.",
  },
  {
    link: '/images/gallery/46.jpeg',
    caption: 'Meetup With <strong> IIT Mandi Director </strong> with <strong> Manipal University students</strong> and <strong>IIIT Allahabad students</strong>',
  },
  {
    link: '/images/gallery/47.jpeg',
    caption: 'Meetup With ohter researcher with <strong> Manipal University students</strong> and <strong>IIIT Allahabad students</strong>',
  },
  {
    link: '/images/gallery/48.jpeg',
    caption: 'Research paper presentation at <strong> IIT Mandi</strong> with <strong> Manipal University students</strong> and <strong>IIIT Allahabad students</strong>',
  },
  {
    link: '/images/gallery/49.jpeg',
    caption: 'Research paper presentation at <strong> IIT Roorkee</strong> with <strong> Manipal University students</strong> and <strong>IIIT Allahabad students</strong>',
  },
  {
    link: '/images/gallery/50.jpeg',
    caption: 'Research paper presentation at <strong> IIT Roorkee</strong> with <strong> Manipal University students</strong> and <strong>IIIT Allahabad students</strong>',
  },


];

export default data;
